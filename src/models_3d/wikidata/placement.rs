...
pub fn prescan(
    ...
    let unique_models: Vec<(String, String, Option<String>)> = {
        let mut set: HashSet<(String, String, Option<String>)> = HashSet::new();

        for c in &candidates {
            if let Some(e) = lookup(&c.placement.qid) {
                set.insert((
                    c.placement.qid.clone(),
                    e.url.clone(),
                    e.bundled_filename.clone(),
                ));
            }
        }
    ...
    let fetched: HashMap<String, Vec<u8>> = unique_models
        .par_iter()
        .filter_map(|(qid, url, bundled_filename)| {
            match fetch_model(url, bundled_filename.as_deref()) {
                Ok(bytes) => Some((url.clone(), bytes)),
                Err(e) => {
                    eprintln!(
                        "{} Wikidata model fetch failed ({} / {}): {e}",
                        "Warning:".yellow().bold(),
                        qid,
                        url
                    );
