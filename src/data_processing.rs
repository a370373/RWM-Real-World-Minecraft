...
/// unchanged (3D models + subways preserved). `RWM_STREAM_TO_DISK=1/0` overrides; constants tunable.
fn should_stream_to_disk(num_regions: usize) -> bool {
    match std::env::var("RWM_STREAM_TO_DISK").ok().as_deref() {
        Some("1") => return true,
        Some("0") => return false,
        _ => {}
    }
...
pub fn generate_world_with_options(
    ...
    // Pre-build a bitmap of pedestrian/path infrastructure.
    let path_mask = highways::collect_path_surface_coords(&elements, &xzbbox, args.scale);
    ...
    let hash_check = std::env::var_os("RWM_BLOCK_HASH").is_some();
    ...
    if use_parallel_tiles {
        ...
        // RWM_NO_BAND forces LPT order even under eviction (debug: isolates ordering effects).
        let band = eviction_active && std::env::var_os("RWM_NO_BAND").is_none();
        if band {
    ...
    // Generate configurable terrain around the authoritative RWM area.
    // RWM data inside xzbbox always remains authoritative.
    // Tell WorldEditor that this outside area is intentional world content,
    // so the Java save phase persists the corresponding regions.
    editor.set_outside_padding(args.outside_padding);
    crate::outside_terrain::generate_outside_terrain(
        &mut editor,
        ground.as_ref(),
        &xzbbox,
        args.outside_padding,
        args.outside_terrain,
    );
    ...
    if wants_map_item {
        ...
        if let Err(e) = crate::map_item::write_branding_map_only(&output_path) {
            eprintln!("Warning: Failed to create RWM map: {e}");
        }
    }
    ...
    if world_format == WorldFormat::JavaAnvil {
        ...
        // Use output_path (the actual "RWM World N" folder holding level.dat), not args.path —
