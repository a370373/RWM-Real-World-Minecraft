...
pub struct Args {
    ...
    /// Terrain generated outside the authoritative RWM bounding box.
    #[arg(long, value_enum, default_value = "normal")]
    pub outside_terrain: OutsideTerrain,

    /// How far the outside terrain extends beyond the RWM bounding box.
    #[arg(long, default_value_t = 128)]
    pub outside_padding: i32,
...
mod tests {
    ...
    fn test_generation_mode() {
        ...
        let base = ["rwm", "--output-dir", tmp_path, "--bbox", "1,2,3,4"];
    ...
    fn test_flags() {
        ...
        let cmd = ["rwm", "--output-dir", tmp_path, "--bbox", "1,2,3,4"];
        let args = Args::parse_from(cmd.iter());
        assert!(!args.debug);
    ...
    fn test_bedrock_flag() {
        ...
        let cmd = ["rwm", "--bedrock", "--bbox", "1,2,3,4"];
        let args = Args::parse_from(cmd.iter());
        assert!(args.bedrock);
    ...
    fn test_required_options() {
        ...
        let cmd = ["rwm", "--path", tmp_path, "--bbox", "1,2,3,4"];
        let args = Args::try_parse_from(cmd.iter()).unwrap();
        assert!(validate_args(&args).is_ok());

        let cmd = ["rwm", "--output-dir", tmp_path, "--file", ""];
        assert!(Args::try_parse_from(cmd.iter()).is_err());
