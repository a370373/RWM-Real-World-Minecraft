...
const STORAGE_KEY = 'rwm-settings';
...
function readStored() {
  ...
  try {
    ...
    console.warn('Stored RWM settings are not valid JSON, ignoring them.');
    clearStored();
    return {};
  }
  ...
  if (parsed.version !== SCHEMA_VERSION) {
    if (typeof parsed.version === 'number' && parsed.version > SCHEMA_VERSION) {
      ...
      console.warn('RWM settings were written by a newer version, using defaults.');
...
function writeStored(values) {
  ...
  try {
    ...
    // once per pixel.
    writesSuppressed = true;
    console.warn('Could not persist RWM settings:', error);
  }
}
