...
async function getLocalization() {
  ...
  const savedLanguage = localStorage.getItem('rwm-language');
...
const SEEN_VERSION_KEY = "rwm-update-seen-version";
  ...
  window.__TAURI__.event.listen("progress-update", (event) => {
    ...
    if (message != "") {
      ...
      if (message.startsWith("Error!")) {
        ...
        window.rwmPreview3D?.setGenerationRunning(false);
        setWorldNameLabel("");
        resetEta();
...
function initSettings() {
  ...
  languageSelect.addEventListener("change", async () => {
    ...
    localStorage.setItem('rwm-language', selectedLanguage);
    ...
    try {
      ...
      if (Array.isArray(rows) && rows.length > 0) {
        ...
        const block =
          ...
          `<p style="font-size: 0.9em;">Permissive-licensed models used to render famous landmarks. Voxelized and rescaled by RWM.</p>` +
...
function initWorldFormatToggle() {
  ...
  const savedFormat = localStorage.getItem('rwm-world-format');
...
function isLuantiEnabled() {
  return localStorage.getItem('rwm-luanti-enabled') === 'true';
}
...
function initLuantiExperimentalToggle() {
  ...
  toggle.addEventListener('change', () => {
    ...
    localStorage.setItem('rwm-luanti-enabled', on ? 'true' : 'false');
    applyRightmost(on);
...
function setWorldFormat(format) {
  ...
  localStorage.setItem('rwm-world-format', format);
...
async function initSavePath() {
  ...
  const saved = localStorage.getItem('rwm-save-path');
  if (saved) {
...
function displayBboxInfoText(bboxText) {
  ...
  if (lng1 === 0 && lat1 === 0 && lng2 === 0 && lat2 === 0) {
    ...
    window.rwmPreview3D?.onBboxCleared();
    return;
  }
  ...
  window.rwmPreview3D?.onBboxChanged(selectedBBox);
}
...
async function startGeneration() {
  try {
    ...
    window.rwmPreview3D?.setGenerationRunning(true);
  } catch (error) {
