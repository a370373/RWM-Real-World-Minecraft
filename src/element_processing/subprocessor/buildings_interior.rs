...
pub fn generate_building_interior(
    ...
    // ============================================================
    // UPGRADE 12 — Building-Type Interior Intelligence
    // READ-ONLY bridge:
    // Existing world/building geometry remains authoritative.
    // Interior intelligence may only consume building information.
    // ============================================================
    let _u12_building_width = max_x - min_x + 1;
    let _u12_building_depth = max_z - min_z + 1;
    let _u12_floor_count = floor_levels.len();

    // Existing interior renderer remains responsible for Minecraft
    // block placement. U12 will supply semantic decisions to it.
    //
    // IMPORTANT:
    // - Do NOT regenerate exterior geometry.
    // - Do NOT create/relocate facade windows here.
    // - Do NOT modify BBox/world reconstruction data.
    // - Existing OSM/Overture-derived building geometry remains immutable.
    //
    // The decision layer will be connected to the existing
    // BuildingProfile / RoomAllocation / FloorPlan pipeline next.
    let _u12_existing_building_geometry = (
        min_x,
        min_z,
        max_x,
        max_z,
