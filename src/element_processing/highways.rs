...
/// Collect all (x, z) coordinates covered by pedestrian/path infrastructure.
/// Uses the exact same Bresenham + highway_block_range geometry as the renderer.
pub fn collect_path_surface_coords(
    elements: &[ProcessedElement],
    xzbbox: &XZBBox,
    scale: f64,
) -> crate::floodfill_cache::PathMaskBitmap {
    let mut bitmap = crate::floodfill_cache::PathMaskBitmap::new(xzbbox);
...
mod tests {
    ...
    fn runway_paints_white_centerline_and_edges_over_gray() {
        let args = Args::parse_from(["rwm", "--bbox", "1,2,3,4"].iter());
