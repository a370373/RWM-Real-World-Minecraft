...
use crate::element_processing::building_intelligence::{
    build_floor_plan, generate_intelligent_building_interior, BuildingContext, BuildingEnvironment,
};
...
// ============================================================================
// Building Intelligence: READ-ONLY existing-window extraction
// ============================================================================
//
// This function ONLY reads the facade-generation rules.
//
// It does NOT:
// - place blocks
// - remove blocks
...
// - modify the BBox
//
// The exterior reconstruction engine remains authoritative.
// Interior Intelligence receives this information as read-only input.
// ============================================================================

fn collect_existing_windows(
    element: &crate::osm_parser::ProcessedWay,
    config: &BuildingConfig,
    bounds: &BuildingBounds,
) -> Vec<crate::element_processing::building_intelligence::input::ExistingWindow> {
    use crate::element_processing::building_intelligence::input::{ExistingWindow, WindowSide};
    ...
    for node in &element.nodes {
        ...
        if let Some(prev) = previous {
            ...
            let dz = current.1 - prev.1;

            let side = if dx.abs() >= dz.abs() {
                if dz >= 0 {
                    WindowSide::South
                } else {
                    WindowSide::North
            ...
            for (bx, _, bz) in points {
                if bx < bounds.min_x || bx > bounds.max_x || bz < bounds.min_z || bz > bounds.max_z
                {
                    continue;
                }

                let max_h = config.start_y_offset + config.building_height;

                for h in (config.start_y_offset + 1)..=max_h {
                    let block = determine_wall_block_at_position_pristine(bx, h, bz, config);

                    if block == config.window_block || block == GLASS {
                        windows.push(ExistingWindow {
                            x: bx,
                            z: bz,
                            width: 1,
                            height: 1,
                            floor: ((h - config.start_y_offset - 1) / 4).max(0),
                            side,
                        });
    ...
    if corner_count > 0 {
        ...
        if args.interior {
            ...
            if !skip_interior && cached_floor_area.len() > 100 {
                ...
                let building_type_enum =
                    crate::element_processing::subprocessor::interior::BuildingType::from_tags(
                        element.tags.get("building").map(String::as_str),
                        element.tags.get("building:use").map(String::as_str),
                        element.tags.get("amenity").map(String::as_str),
                        element.tags.get("shop").map(String::as_str),
                        element.tags.get("office").map(String::as_str),
                        element.tags.get("healthcare").map(String::as_str),
                    );

                let building_context = BuildingContext {
                ...
                let (nearby_road_distance, nearby_road_position) = {
                    let mut best: Option<(i32, (i32, i32))> = None;

                    for &(x, z) in &cached_floor_area {
                        if let Some((rx, rz)) =
                            crate::element_processing::get_nearest_road_block(x, z, 8, road_mask)
                        {
                            let distance = (rx - x).abs() + (rz - z).abs();

                            if best.map_or(true, |(current, _)| distance < current) {
                                best = Some((distance, (rx, rz)));
                            }
                        }
                    }

                    match best {
                        Some((distance, position)) => (Some(distance), Some(position)),
                        None => (None, None),
                    }
                };

                let building_snapshot =
    crate::element_processing::building_intelligence::input::BuildingSnapshot {
        building_type: building_type_enum,
        min_x: bounds.min_x,
        min_z: bounds.min_z,
        max_x: bounds.max_x,
        ...
        // READ-ONLY reconstruction inputs.
        //
        // The interior intelligence engine does NOT generate or modify
        // exterior geometry. Window extraction will be connected separately.
        windows: {
            let mut windows = collect_existing_windows(
                element,
                &config,
                &bounds,
            );

            // Supplement the authoritative reconstructed windows with
            // explicitly mapped real-world window semantics.
            let mapped_windows =
                crate::element_processing::building_intelligence::input::collect_existing_windows(
                    &element.nodes,
                    bounds.min_x,
                    bounds.min_z,
                    bounds.max_x,
        ...
        doors: crate::element_processing::building_intelligence::input::collect_existing_doors(
            &element.nodes,
            bounds.min_x,
            bounds.min_z,
            bounds.max_x,
        ...
        // Preserve the original OSM / Overture semantic information.
        osm_tags: element
            .tags
            .iter()
