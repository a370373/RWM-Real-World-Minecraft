...
/// RWM branding image, placed as a locked map at spawn.
...
pub fn write_map_item(
    ...
    let branding_dat = image_map_dat_or_blank(&[], data_version);
...
/// Writes only the RWM branding map (the world's first map) when the preview map is off.
...
mod tests {
    ...
    fn writes_branding_map_beside_preview_but_world_only() {
        ...
        // Preview is map 0; the RWM branding map is the next id, 1.
