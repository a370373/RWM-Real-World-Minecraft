...
/// Overture Maps public S3 bucket.
/// Releases are discovered dynamically; no release version is hard-coded.
const OVERTURE_S3_ROOT: &str = "https://overturemaps-us-west-2.s3.amazonaws.com";
/// Overture Maps official STAC catalog root.
const OVERTURE_STAC_ROOT: &str = "https://stac.overturemaps.org";
/// Overture Maps public STAC catalog endpoint.
...
/// Discover Overture releases from the official STAC catalog.
///
/// Overture now treats STAC as the authoritative release-discovery mechanism.
/// We deliberately do NOT depend on `collections.parquet`, which is no longer
/// a reliable release discovery endpoint.
///
/// Fallback order:
///   1. STAC root catalog
///   2. STAC child release catalogs
///   3. Release versions already returned by STAC
///
/// The returned list is newest-first.
/// Discover Overture releases from the official public S3 bucket.
///
/// Overture's current public distribution is:
///
///   release/<release>/theme=buildings/type=building/*
///
/// We intentionally avoid the old `collections.parquet` endpoint and the
/// currently unavailable STAC HTTP endpoint. The public S3 bucket is the
/// authoritative data distribution used by Overture's own tooling.
///
/// The returned list is newest-first.
fn discover_overture_releases(
    client: &Client,
    debug: bool,
) -> Result<Vec<String>, Box<dyn std::error::Error>> {
    discover_overture_releases_from_s3(client, debug)
}

/// List only the building partitions needed by the selected release.
///
/// IMPORTANT:
/// This does NOT download the building dataset. It only asks S3 for the
/// object keys under the buildings/building Hive partition. Actual spatial
/// filtering happens later through Parquet row-group statistics and HTTP
/// Range requests.
/// List only Overture building partitions whose STAC bbox intersects
/// the requested RWM bounding box.
///
/// Overture's official STAC layout exposes the building collection and
/// individual STAC Items. Each Item describes one GeoParquet partition and
/// contains its geographic bbox plus the AWS data URL.
///
/// This lets RWM perform spatial filtering BEFORE touching the Parquet files:
///
///     RWM bbox
///        ↓
///     STAC partition bbox
...
///     intersects?
///        ↓
///     only matching Parquet URLs
///
/// The Parquet row-group filtering in `process_partition_file()` remains as
...
fn list_partition_files(
    ...
    let releases = discover_overture_releases(client, debug)?;
    ...
    for release in releases {
        /*
         * Overture's official STAC layout:
         *
         ...
         *         collection.json
         *
         * The collection contains links to partition STAC Items.
         * Each Item contains:
         *
         *   bbox
         *   assets.aws.href
         *
         * We use those metadata bboxes to select only partitions
         * intersecting the requested RWM bbox.
         *
         * IMPORTANT:
         * We do NOT download any parquet here.
         */

        let collection_url =
            format!("https://stac.overturemaps.org/{release}/buildings/building/collection.json");

        if debug {
            println!("Overture STAC: checking building collection for release {release}");
        }

        let response = match client.get(&collection_url).send() {
            Ok(r) => r,
            Err(e) => {
                last_error = e.to_string();
                eprintln!(
                    "{} Overture STAC release {} failed: {}; trying older release...",
                    "Warning:".yellow().bold(),
                    release,
                    last_error
                );
        ...
        if !response.status().is_success() {
            last_error = format!(
                "building STAC collection returned HTTP {}",
                response.status()
            );
        ...
        let collection: serde_json::Value = match response.json() {
            Ok(v) => v,
            Err(e) => {
                last_error = format!("failed to parse STAC collection: {e}");
        ...
        if let Some(links) = collection.get("links").and_then(|v| v.as_array()) {
            for link in links {
                if link.get("rel").and_then(|v| v.as_str()) != Some("item") {
                    continue;
                }
                ...
                let item_url = if href.starts_with("http://") || href.starts_with("https://") {
                    href.to_string()
                } else {
                    format!(
                        "https://stac.overturemaps.org/{release}/buildings/building/{}",
                        href.trim_start_matches("./")
                    )
                };

                item_urls.push(item_url);
            }
        }
        ...
        if debug {
            println!(
                "Overture STAC: discovered {} building partition metadata items",
                item_urls.len()
            );
        }
        ...
        if item_urls.is_empty() {
            last_error =
                format!("building STAC collection for release {release} contains no items");
            continue;
        }
        ...
        for item_url in item_urls {
            let response = match client.get(&item_url).send() {
                Ok(r) => r,
                Err(e) => {
                    if debug {
                        eprintln!("Warning: failed to fetch Overture STAC item: {e}");
                    }
                    continue;
                }
            };
            ...
            if !response.status().is_success() {
                if debug {
                    eprintln!(
                        "Warning: Overture STAC item returned HTTP {}",
                        response.status()
                    );
                }
                continue;
            ...
            let item: serde_json::Value = match response.json() {
                Ok(v) => v,
                Err(e) => {
                    if debug {
                        eprintln!("Warning: failed to parse Overture STAC item: {e}");
                    }
                    continue;
                }
            };
             ...
             */
            let Some(b) = item.get("bbox").and_then(|v| v.as_array()) else {
                continue;
            };
            ...
            let (Some(item_min_lng), Some(item_min_lat), Some(item_max_lng), Some(item_max_lat)) =
             ...
             */
            let overlaps = item_min_lng <= target_max_lng
                && item_max_lng >= target_min_lng
                && item_min_lat <= target_max_lat
                && item_max_lat >= target_min_lat;
            ...
            /*
             * Prefer AWS parquet asset.
             */
            let mut parquet_url = None;

            if let Some(assets) = item.get("assets").and_then(|v| v.as_object()) {
                for key in ["aws", "data", "parquet"] {
                    if let Some(asset) = assets.get(key) {
                        if let Some(href) = asset.get("href").and_then(|v| v.as_str()) {
                            if href.ends_with(".parquet") {
                                parquet_url = Some(href.to_string());
                                break;
                            }
                        }
                    }
                ...
                /*
                 * Fallback: search every asset for parquet.
                 */
                if parquet_url.is_none() {
                    for asset in assets.values() {
                        if let Some(href) = asset.get("href").and_then(|v| v.as_str()) {
                            if href.contains(".parquet") {
                                parquet_url = Some(href.to_string());
                                break;
                            }
                        }
        ...
        if debug {
            println!(
                "Overture STAC: {} partition(s) intersect requested bbox",
                matching_urls.len()
            );

            println!(
                "Overture bbox: min=({}, {}), max=({}, {})",
                target_min_lat, target_min_lng, target_max_lat, target_max_lng
            );
        }

        /*
         * Zero matching partitions is a valid result.
         */
        return Ok(matching_urls);
    }

    Err(format!("All discovered Overture releases failed: {last_error}").into())
}

fn discover_overture_releases_from_s3(
    client: &Client,
    debug: bool,
) -> Result<Vec<String>, Box<dyn std::error::Error>> {
    let url = format!("{OVERTURE_S3_ROOT}/?list-type=2&delimiter=/&prefix=release/");

    let response = client.get(&url).send()?;

    if !response.status().is_success() {
        return Err(format!(
            "Overture S3 release listing failed: HTTP {}",
            response.status()
        )
        .into());
    }
    ...
    for part in body.split("<CommonPrefixes>") {
        if let Some(end) = part.find("</CommonPrefixes>") {
            let group = &part[..end];

            if let Some(start) = group.find("<Prefix>release/") {
                let value = &group[start + "<Prefix>release/".len()..];

                if let Some(end_prefix) = value.find("</Prefix>") {
                    let release = value[..end_prefix].trim_end_matches('/');
    ...
    if debug {
        println!("Overture S3: discovered {} release(s)", releases.len());
    ...
    if releases.is_empty() {
        return Err("No Overture releases discovered from official S3 bucket".into());
    }
