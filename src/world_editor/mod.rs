...
impl<'a> WorldEditor<'a> {
    ...
    /// Preview map in a frame at spawn, with the RWM map one block below.
    ...
    /// RWM map at spawn on a floating block, used when the preview map is off.
    ...
    /// Set the amount of intentionally generated terrain outside the
    /// authoritative RWM bounding box that should be persisted.
    #[inline]
    pub fn set_outside_padding(&mut self, padding: i32) {
        self.outside_padding = padding.max(0);
    }

    /// Sets a block outside the authoritative RWM bounding box.
    ///
    /// Used exclusively by the outside-terrain generator. Unlike the normal
    /// block-writing APIs, this intentionally does not enforce `xzbbox`.
    #[inline]
    pub fn set_outside_block_absolute(&mut self, block: Block, x: i32, absolute_y: i32, z: i32) {
        if self.is_region_flushed(x, z) {
            return;
        }

        self.world
            .fill_column(x, z, absolute_y, absolute_y, block, false);
    }

    /// Fills an outside-terrain column using absolute Minecraft Y coordinates.
    #[inline]
    pub fn fill_outside_column(&mut self, x: i32, z: i32, y_min: i32, y_max: i32, block: Block) {
        if self.is_region_flushed(x, z) {
            return;
        }

        self.world.fill_column(x, z, y_min, y_max, block, false);
    }
