...
impl<'a> WorldEditor<'a> {
    ...
    pub(super) fn save_java(&mut self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        // Persist the authoritative RWM bbox plus intentionally generated
        // outside terrain. Outside padding is real world content, not tile halo.
        //
        // A region at (rx, rz) covers blocks:
        // [rx*512 .. rx*512+511] × [rz*512 .. rz*512+511].
        let save_min_x = self.xzbbox.min_x() - self.outside_padding;
        let save_max_x = self.xzbbox.max_x() + self.outside_padding;
        let save_min_z = self.xzbbox.min_z() - self.outside_padding;
        let save_max_z = self.xzbbox.max_z() + self.outside_padding;

        let min_region_x = save_min_x.div_euclid(512);
        let max_region_x = save_max_x.div_euclid(512);
        let min_region_z = save_min_z.div_euclid(512);
        let max_region_z = save_max_z.div_euclid(512);
        ...
        self.world
            ...
            .for_each(|((region_x, region_z), region_to_modify)| {
                // Skip regions outside the authoritative bbox + intentionally
                // generated outside-terrain padding.
...
fn write_region_to_disk(
    ...
    //
    // IMPORTANT:
    // Empty chunks inside the authoritative RWM bbox still receive the
    // traditional base chunk.
    //
    // Empty chunks outside the authoritative bbox but inside the intentional
    // outside-padding area are valid Void terrain and MUST remain empty.
    //
    // This prevents create_base_chunk() from turning --outside-terrain void
    // back into the default superflat/base terrain.
    //
    // Skip entirely when region already has all 1024 chunks.
        ...
        for chunk_x in 0..32 {
            for chunk_z in 0..32 {
                if !region_to_modify.chunks.contains_key(&(chunk_x, chunk_z)) {
                    ...
                    let chunk_min_x = abs_chunk_x * 16;
                    let chunk_max_x = chunk_min_x + 15;
                    let chunk_min_z = abs_chunk_z * 16;
                    let chunk_max_z = chunk_min_z + 15;

                    let inside_authoritative_bbox = chunk_max_x >= xzbbox.min_x()
                        && chunk_min_x <= xzbbox.max_x()
                        && chunk_max_z >= xzbbox.min_z()
                        && chunk_min_z <= xzbbox.max_z();

                    let outside_min_x = xzbbox.min_x() - outside_padding;
                    let outside_max_x = xzbbox.max_x() + outside_padding;
                    let outside_min_z = xzbbox.min_z() - outside_padding;
                    let outside_max_z = xzbbox.max_z() + outside_padding;

                    let inside_save_area = chunk_max_x >= outside_min_x
                        && chunk_min_x <= outside_max_x
                        && chunk_max_z >= outside_min_z
                        && chunk_min_z <= outside_max_z;

                    // Intentionally empty outside terrain = real Void.
                    if inside_save_area && !inside_authoritative_bbox {
                        continue;
                    }
