...
impl BedrockWriter {
    ...
    fn package_mcworld(&self) -> Result<(), BedrockSaveError> {
        ...
        if self.extend_build_height {
            ...
            writer.add_directory("behavior_packs/rwm_tall/", options)?;
            writer.add_directory("behavior_packs/rwm_tall/dimensions/", options)?;

            writer.start_file("behavior_packs/rwm_tall/manifest.json", options)?;
            writer.write_all(BP_MANIFEST)?;

            writer.start_file("behavior_packs/rwm_tall/dimensions/overworld.json", options)?;
...
mod tests {
    ...
    fn writes_mcworld_with_tall_behavior_pack_when_extended() {
        ...
        assert!(
            entries.contains(&"behavior_packs/rwm_tall/manifest.json".to_string()),
            "missing BP manifest: {entries:?}"
        );
        assert!(
            entries.contains(&"behavior_packs/rwm_tall/dimensions/overworld.json".to_string()),
        ...
        let manifest_bytes = {
            let mut f = archive
                .by_name("behavior_packs/rwm_tall/manifest.json")
