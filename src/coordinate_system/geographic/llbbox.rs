...
mod tests {
    ...
    fn test_from_str_commas() {
        const TEST_LLBBOX_STR: &str = "9.927928,54.627053,9.937563,54.634902";

        let bbox_result = LLBBox::from_str(TEST_LLBBOX_STR);
        assert!(bbox_result.is_ok());

        let rwm_correct: LLBBox = LLBBox {
            min: LLPoint::new(9.927928, 54.627053).unwrap(),
            max: LLPoint::new(9.937563, 54.634902).unwrap(),
        };

        assert_eq!(bbox_result.unwrap(), rwm_correct);
    }

    #[test]
    fn test_from_str_spaces() {
        const TEST_LLBBOX_SPACE_STR: &str = "9.927928 54.627053 9.937563 54.634902";

        let bbox_result = LLBBox::from_str(TEST_LLBBOX_SPACE_STR);
        assert!(bbox_result.is_ok());
