...
/// growing pile of empty "RWM World N" folders.
...
fn add_localized_world_name(world_path: PathBuf, bbox: &LLBBox) -> PathBuf {
    ...
    // Only modify if it's an RWM world and doesn't already have an area name
    if !current_name.starts_with("RWM World ") || current_name.contains(": ") {
        return world_path;
    }
...
fn gui_get_world_map_data(world_path: String) -> Result<Option<WorldMapData>, String> {
    ...
    let map_path = world_dir.join("rwm_world_map.png");
        ...
        if let Err(e) = tokio::task::spawn_blocking(move || {
            ...
            let (generation_path, level_name) = match world_format {
                ...
                WorldFormat::LuantiWorld => {
                    ...
                    let mut counter = 1;
                    let world_name = loop {
                        let candidate = format!("RWM Luanti World {counter}");
