...
/// Falls back to Desktop/RWM Luanti Worlds if no path can be resolved.
...
pub fn build_bedrock_output(bbox: &LLBBox, output_dir: PathBuf) -> (PathBuf, String) {
    ...
    let filename = format!("RWM {safe_name}.mcworld");
    let lvl_name = format!("RWM World: {safe_name}");
    (output_dir.join(&filename), lvl_name)
}

/// Creates a new Java Edition world in the given base directory.
///
/// Generates a unique "RWM World N" name, creates the directory structure
...
pub fn create_new_world(base_path: &Path) -> Result<String, String> {
    ...
    // Check for both "RWM World X" and "RWM World X: Location" patterns
    let mut counter: i32 = 1;
    let unique_name: String = loop {
        let candidate_name: String = format!("RWM World {counter}");
        ...
        // Check for worlds with location suffix (RWM World X: Location)
        let location_pattern = format!("RWM World {counter}: ");
...
pub const TALL_DATAPACK_NAME: &str = "rwm_tall";
